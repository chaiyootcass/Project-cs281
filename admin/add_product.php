    ...
    if ((!$product_name) || (!$product_name) || (!$product_name) || (!$category) || (!$product_name) || (!$product_name) || (!$product_name)) {
        ...
        if ($_FILES['fileField']['tmp_name'] != "") {
            ...
            if ($_FILES['fileField']['size'] > $maxfilesize) {
                ...
                $sql = mysqli_query($con, "INSERT INTO product(name, description, price, quantity, date_added,category) VALUES('$product_name','$product_description','$product_price','$product_quantity', now(),'$category')");
                $id = mysqli_insert_id($con);
                $newname = $id . ".jpg";
                $place_file = move_uploaded_file($_FILES['fileField']['tmp_name'], "../products/" . $newname);
